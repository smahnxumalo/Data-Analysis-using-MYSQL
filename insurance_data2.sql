...
-- 1. select all columns for all patience 
select * from insurance_data;

-- 2. Display the avarage claim amount for patience in each region.
select region, avg(claim) as avg_claim from insurance_data
group by region;

-- 3. select the maximum and minimum BMI in the table
select max(bmi) as max_bmi, min(bmi) as min_bmi from insurance_data 

-- 4. Select the PatientId , age and BMI for patience with the BMI between 40 and 50 
select PatientID, age, bmi from insurance_data where bmi between 40 and 50;

-- 5. select number of smokers in each region
select region, count(PatientID) from insurance_data where smoker = "Yes"  group by region;

-- 6. What is the avarage claim amount for Patients who are both diabetic and smokers 
select avg(claim) as avg_claim from insurance_data where diabetic ="Yes" and smoker="Yes";

-- 7. Retrieve all patients who have BMI greater than the avarage BMI of patience who are smokers 
...
-- 8.  Select the avarage claim amount for patients in each age group 
select 
